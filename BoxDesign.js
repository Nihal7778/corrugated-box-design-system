...
import { Container, Typography, Paper, Table, TableBody, TableCell, TableHead, TableRow, Button, IconButton, Dialog, DialogActions, DialogContent, DialogTitle, TextField } from '@mui/material';
...
function BoxDesign() {
  ...
  const handleDeleteClick = async (id) => {
    try {
      await axios.post(`http://localhost:5001/boxdesign/delete`, { id });
  ...
  return (
    <Container>
      ...
      <Button startIcon={<Add />} onClick={handleAddClick} color="primary">
        Add Box Design
      </Button>
      <Paper>
        <Table>
          ...
          <TableBody>
            {boxDesigns.map((design) => (
              <TableRow key={design.id}>
                <TableCell>{design.sales_person_code}</TableCell>
                <TableCell>{design.customer_code}</TableCell>
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)}>
        ...
        <DialogContent>
          <TextField
            ...
            value={currentDesign.sales_person_code}
          ...
          <TextField
            ...
            label="Customer Code"
            name="customer_code"
            value={currentDesign.customer_code}
            onChange={handleEditChange}
            fullWidth
          />
          <TextField
